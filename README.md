...
# 站点的语言标识码（表示当前站点使用的语言）
# 默认为英文（en）
languageCode = "en"

# 站点内容默认语言（用于控制主题内词条以何种语言展示）
# 目前主题内只支持：中文（zh）和英文（en）
# 默认为英文（en）
defaultContentLanguage = "en"

# 文章内容的版权信息
# 这个内容将展示在页脚形如：2024 © copyright
# 版权信息支持 Markdown 语法
...
[params]
    ...
    # 站点图标（按照下面的命名方式将指定大小的图标放在自己站点的 "static" 目录中）
    logo = "/logo.svg"
    ...
    # 文章预览页面展示精选文章样式（对应文章的frontmatter中要配置featured: true）
    ...
    # 无限滚动
    # 无限滚动模式替换分页模式（需要在[outputs]中配置输出JSON格式）
...
[menu]
    ...
    # 图标菜单
    [[menu.icons]]
        identifier = "github" # identifier 用于在主题目录的data/supported_icons.yml文件中获取内置的图标
        name = "GitHub"
...
如果需要在页脚展示一些链接信息，那么可以在你的站点中新建 `data/footer_columns.yml` 配置文件，同时在站点配置中开启 `enableFooterColumns = true` 。然后按照下面的示例配置需要展示的链接信息即可。
...
## 巨幕
可以在你的站点新建 `data/jumbotron.yml` 文件来配置添加一个在首页开始前的巨幕，同时在站点配置中开启 `enableJumbotron = true` 。巨幕中需要的资源文件需要放在站点的 `static` 目录中。下面是示例配置：
...
# 文章参数
每个文章都会包含一些参数在 frontmatter 中，下面是一些参数的示例：
...
- `featured`: 默认: false，标注文章是否为精选。
- `comments`: 默认: true，是否开启文章评论(默认使用Disqus)。
- `showMetaInfo`: 默认: true，是否展示日期、字数、阅读时间等信息（适用于非文章页面）。
- `showTitle`: 默认: true，是否隐藏标题。
- `norss`: 默认: false，是否添加到RSS。
- `nosearch`: 默认: false，是否可搜索。
- `toc`: 默认: false，是否展示文章内容目录。
